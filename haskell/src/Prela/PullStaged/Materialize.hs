...
-- continuation-shaped implementation remain private to this module.
--
            ...
            [|| case $$state of
                  (!dictionaryMap, !items, !nextCode) ->
                    let !i = idIndex $$entity
                        !item = $$value
                    in if 0 <= i && i < cap
                       then case Map.lookup item dictionaryMap of
                         Just code -> do
                           UMV.write codes i code
                           writeArray seen i True
                           return (dictionaryMap, items, nextCode)
                         Nothing -> do
                           UMV.write codes i nextCode
                           writeArray seen i True
                           return (Map.insert item nextCode dictionaryMap,
                                   item : items, nextCode + 1)
                       else return (dictionaryMap, items, nextCode) ||])

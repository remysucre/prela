...
-- instead of degrading to a `Drive` that has to buffer.
...
data Drive k v
  = Lin (Producer k v)
  | forall k' v'. Bind (Drive k' v') (k' -> v' -> Drive k v)
  | Cat (Drive k v) (Drive k v)

-- | A relation in probed position. Applying a key returns a pull stream of
-- matching values; the stream's own key is `()` because the supplied key is
-- already known.
newtype Probe k v = Probe { at :: k -> Drive () v }
...
mapkVS :: (d -> r -> d') -> Drive d r -> Drive d' r
...
-- | Swap key and value. Driven only, and free: probing an inverse means
-- building a reverse index, which is "Prela.Pull.Materialize"'s `inv`.
...
-- | Run a stream only if a test decided once (not per row) passes. Used by
-- `diff` in probed position, where the test is on the key.
whenS :: Bool -> Drive d r -> Drive d r
...
sfoldWhile :: (acc -> Bool) -> (acc -> d -> r -> acc) -> acc -> Drive d r -> acc

# `@generated` engine: walk a *prepared* plan's type at codegen time and emit a
# flat, fused loop. Mode is already resolved into the types (`prepare` lowered
# every index/cache node to a concrete prepared struct — `InvStream`/`InvIndexed`,
# `FoldP`/`DenseFoldP`, `MatStream`/`MatProbed`, …), so the emitters read concrete
# fields and never build anything: all index/cache construction is delegated to
# `prepare`, which runs once, bottom-up, before the generated scan.
#
# The emitters are the type-level CPS the value-level `drive`/`probe` specs: the
# `body` Expr is the continuation, threaded over the plan *type* the way `k` is
# threaded over the *value*. The structural mode-propagation (drive→probe on a
# `Compose` rhs, etc.) is reproduced here; the leaf index modes are pre-baked by
# `prepare`. Entry is `_exec_drive(prepared_plan, sink)`; `execute_drive` /
# `_vals` prepare first, then call it.

using Main: Prela
import Main.Prela: Query, Unary, Universe, Compose, Filter, Restrict, Diff, Prod,
                   Map, LeftConj, Disj, FnP, EqP, InP,
                   VecRel, SparseRel, MultiRel, MapRel, UnaryVec, Bitset,
                   InvStream, InvIndexed, LCStream, LCIndexed,
                   MatStream, MatProbed, MatSetStream, MatSetProbed,
                   FoldP, DenseFoldP, ScalarP, Scalar,
                   ID, _denseidx, _densebox, member, drive, probe, probe_any
...
# =====================================================================
# Entry points. `_exec_*` are @generated over the PREPARED plan type; the
# public `execute*` wrappers run `prepare` first.
# =====================================================================
...
execute_count(q) = _exec_count(Prela.prepare(q, Prela.Driven()))

# Scalar tops fold during `prepare` (→ ScalarP); just read the value.
execute(q::Scalar) = Prela.prepare(q, Prela.Driven()).value

# Drop-in override: route the `_prepared` drivers through the engine. The
# argument is already prepared (the `_vals` / `_vals_tpch` wrappers prepared it),
# so we call `_exec_drive` directly — no second prepare.
if isdefined(Main, :_vals_tpch_prepared)
    function Main._vals_tpch_prepared(pq, sort_by, limit, row)
        rows = Tuple{Any, Any}[]
        _exec_drive(pq, (x, y) -> push!(rows, (x, y)))
...
if isdefined(Main, :_vals_prepared)
    function Main._vals_prepared(pq)
        cur = Ref{Any}(nothing)
        emit = y -> begin
            fl = Main._flatten(y)
            c = cur[]
        ...
        if Prela._rangeof(pq) === Tuple{}
            _exec_drive(pq, (x, _) -> emit(x))
        else
            _exec_drive(pq, (_, y) -> emit(y))
        end
        c = cur[]

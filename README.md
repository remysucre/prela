...
It is implemented as a library of _query combinators_ (think [parser combinators](https://en.wikipedia.org/wiki/Parser_combinator)),
...
You can also think of `data.with(...).text()` and `company.with(...).name()` parts as _subqueries_
which require special syntax in SQL, but are just subexpressions in Prela.
And instead of explicit conditions,
joins in Prela are reflected by the _structure_ of the query in a navigational style.
...
100 years older than the familiar _relational_ algebra,
called (confusingly) [_relation_ algebra](https://en.wikipedia.org/wiki/Relation_algebra).
...
In TAR (and therefore Prela), everything is a _binary_ relation,
and a query is built up with operators that take in and produce binary relations,
which I call _relation combinators_.
...
the two coincide and `movie` is the _identity_ relation;
...
Next, `.with()` is the _restriction_ combinator:
...
The `.select()` combinator is _relational composition_.
...
namely the _product_ combinator `.and()`.
...
Now, back to the query: `r.group_by(s)` groups the _rows_ of `r`
...
obtained by plain navigation _after_ grouping:
...
In other words, a Prela query _is_ also a query plan.

...
/// `query.sql`, and the JOB name `prela::job_queries::typed_queries` answers
...
mod tests {
    ...
    fn registry_contains_every_job_sql_query() {
        ...
        let database = crate::generate::Database {
            tables: schema::SCHEMA
                .tables
                .iter()
                .map(|&entity| crate::generate::Table {
                    entity,
                    rows: Vec::new(),
                })
                .collect(),
        };
        let connection = crate::runner::production::sql_database(&crate::sql::to_sql(
            &schema::SCHEMA,
            &database,
        ))
        .unwrap();
        let answers = crate::runner::production::run(&connection).unwrap();
        let mut production = answers
            .keys()
            .map(String::as_str)

...
//! [`Schema`](crate::schema::Schema). Its internal [`Database`] model is
//! rendered as SQL; the Prela importer reads the resulting SQL database.
//!
...
/// SQL rendering writes it as `NULL`; the importer then omits the corresponding
/// relation fact when reading the stored value.
...
/// Generation and hand-built fixture model, rendered to SQL before execution.
#[derive(Clone, Debug, PartialEq)]
pub struct Database {
...
/// Generate replayable SQL through structured, shrinkable draws. The row model
/// is an implementation detail of generation, not an input to the Prela importer.
pub fn sql_generator(schema: &'static Schema) -> impl hegel::Generator<String> + Send + Sync {
    hegel::compose!(|tc| {
        let database = tc.draw(generator(schema));
        crate::sql::to_sql(schema, &database)
    })
}

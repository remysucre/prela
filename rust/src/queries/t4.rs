...
fn q16ad(db: &'static Job, lo: i64) -> impl Drive<R: Row> {
    ...
    movie.with(company.select(country).eq("[us]")
       ...
       .select(cast.select(person).select(alias).select(akaname_text)
          .and(title))
}

fn q16a(db: &'static Job) -> impl Drive<R: Row> { q16ad(db, 50) }
fn q16d(db: &'static Job) -> impl Drive<R: Row> { q16ad(db, 5) }
...
fn q17b(db: &'static Job) -> impl Drive<R: Row> { q17_any_co(db, r"^Z") }
fn q17c(db: &'static Job) -> impl Drive<R: Row> { q17_any_co(db, r"^X") }
fn q17d(db: &'static Job) -> impl Drive<R: Row> { q17_any_co(db, r"Bert") }
fn q17f(db: &'static Job) -> impl Drive<R: Row> { q17_any_co(db, r"B") }
...
fn ib_18a(db: &'static Job) -> impl Query<R = &'static str, D = Id<Movie>> + Drive + Probe {
    ...
    info.with(info_ty.select(infotype_text).eq("budget")).select(info_info)
}
...
fn q18a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let movie = db.movie.all();
    movie.with(ib_18a(db)
          .and(cast.select(cast_note.is_in(["(producer)", "(executive producer)"])
                      .and(person.select(gender.eq("m")
                                    .and(person_name.rx(r"Tim")))))))
       .select(ib_18a(db)
...
fn q18b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let movie = db.movie.all();
    movie.with(info.with(gf_18b(db))
          .and(production_year.ge(2008))
          .and(production_year.le(2014))
          .and(cast.select(cast_note.is_in(writer5())
...
fn q18c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Person { gender, .. } = &db.person;
    let movie = db.movie.all();
    movie.with(info.with(gf_18c(db))
          .and(cast.select(cast_note.is_in(writer5())
                      .and(person.select(gender.eq("m"))))))

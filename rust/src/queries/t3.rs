...
fn q7a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let AkaName { text: akaname_text, .. } = &db.aka_name;
    ...
    let PersonInfo { ty: personinfo_ty, note: personinfo_note, .. } = &db.person_info;
    let movie = db.movie.all();
    movie.with(production_year.ge(1980)
          .and(production_year.le(1995))
          .and(linked_by.select(movielink_ty).select(linktype_text).eq("features")))
       .select(cast.select(person
                        .with(alias.select(akaname_text).rx(r"a")
                         ...
                         .and(bio.select(personinfo_ty.select(infotype_text).eq("mini biography")
...
fn bio_filter_7c(db: &'static Job) -> impl Query<D = Id<PersonInfo>> + Probe {
    let InfoType { text: infotype_text, .. } = &db.info_type;
    let PersonInfo { ty: personinfo_ty, note: personinfo_note, .. } = &db.person_info;
    personinfo_ty.select(infotype_text).eq("mini biography")
        .and(personinfo_note)
}
...
fn q7c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let MovieLink { ty: movielink_ty, .. } = &db.movie_link;
    let Person { name: person_name, gender, alias, bio, name_pcode_cf, .. } = &db.person;
    let PersonInfo { info: personinfo_info, .. } = &db.person_info;
    let movie = db.movie.all();
    movie.with(production_year.ge(1980)
          .and(production_year.le(2010))
          .and(linked_by.select(movielink_ty).select(linktype_text).is_in(
                  ["references", "referenced in", "features", "featured in"])))
       .select(cast.select(person
                        .with(alias.select(akaname_text).rx(r"a|^A")
                         .and(name_pcode_cf.ge("A"))
                         .and(name_pcode_cf.le("F"))
                        ...
                        .select(person_name
                           .and(bio.with(bio_filter_7c(db)).select(personinfo_info)))))
}
...
fn q8a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Person { name: person_name, alias, .. } = &db.person;
    ...
    movie.with(company.select(country.eq("[jp]")
       ...
       .select(cast
             .with(cast_note.eq("(voice: English version)")
              .and(role.select(roletype_text).eq("actress"))
              .and(person.with(person_name.rx(r"Yo")
                          .and(person_name.nrx(r"Yu")))))
             .select(person).select(alias).select(akaname_text)
          .and(title))
}
...
fn q8cd(db: &'static Job, role_: &'static str) -> impl Drive<R: Row> {
    ...
    movie.with(company.select(country).eq("[us]"))
       .select(cast.with(role.select(roletype_text).eq(role_))
             .select(person).select(alias).select(akaname_text)
          .and(title))
}

fn q8c(db: &'static Job) -> impl Drive<R: Row> { q8cd(db, "writer") }
fn q8d(db: &'static Job) -> impl Drive<R: Row> { q8cd(db, "costume designer") }
...
fn q10a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, cast, .. } = &db.movie;
    let Cast { role, note: cast_note, character, .. } = &db.cast;
    let Character { text: character_text, .. } = &db.character;
    let Company { country, .. } = &db.company;
    let RoleType { text: roletype_text, .. } = &db.role_type;
    let movie = db.movie.all();
    movie.with(company.select(country).eq("[ru]")
          .and(production_year.gt(2005)))
       .select(cast
             .with(cast_note.rx(r"\(voice\)")
...
fn q10c(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, cast, .. } = &db.movie;
    let Cast { note: cast_note, character, .. } = &db.cast;
    let Character { text: character_text, .. } = &db.character;
    let Company { country, .. } = &db.company;
    let movie = db.movie.all();
    movie.with(company.select(country).eq("[us]")

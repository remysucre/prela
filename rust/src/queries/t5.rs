...
fn q20a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(complete_cast.select(subject.select(compcasttype_text).eq("cast")
          ...
          .and(cast.select(character.select(character_text.nrx(r"Sherlock")
...
fn q20c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(complete_cast.select(subject.select(compcasttype_text).eq("cast")
          ...
          .and(kind.select(kind_text).eq("movie"))
          .and(production_year.gt(2000)))
       .select(cast.with(character.select(character_text).rx(r"[Mm]an"))
             .select(person).select(person_name)
          .and(title))
}
...
fn q21a(db: &'static Job) -> impl Drive<R: Row> { q21(db, nordic8(), 1950, 2000) }
fn q21b(db: &'static Job) -> impl Drive<R: Row> { q21(db, vec!["Germany", "German"], 2000, 2010) }
fn q21c(db: &'static Job) -> impl Drive<R: Row> { q21(db, nordic9(), 1950, 2010) }
...
fn q23a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let InfoType { text: infotype_text, .. } = &db.info_type;
    let movie = db.movie.all();
    movie.with(complete_cast.select(status).select(compcasttype_text).eq("complete+verified")
...
fn q23b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(complete_cast.select(status).select(compcasttype_text).eq("complete+verified")
          ...
          .and(k_23ab(db))
          .and(keyword.select(keyword_text).is_in(["nerd", "loner", "alienation", "dignity"]))
          .and(production_year.gt(2000)))
       .select(k_23ab(db).and(title))
}
...
fn q24a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(company.select(country).eq("[us]")
          ...
          .and(keyword.select(keyword_text).is_in(["hero", "martial-arts", "hand-to-hand-combat"]))
...
fn q24b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(company.select(country.eq("[us]")
          ...
          .and(info.select(info_ty.select(infotype_text).eq("release dates")
                      .and(info_info.rx(r"^Japan:.*201|^USA:.*201"))))
          .and(keyword.select(keyword_text).is_in(["hero", "martial-arts", "hand-to-hand-combat", "computer-animated-movie"]))
          .and(production_year.gt(2010))
          .and(title.rx(r"^Kung Fu Panda")))
       .select(cast
             .with(cast_note.is_in(voice4())
...
fn q25a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let movie = db.movie.all();
    movie.with(info.with(gf_25ab(db))
          .and(keyword.select(keyword_text).is_in(["murder", "blood", "gore", "death", "female-nudity"])))
...
fn q25c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Person { name: person_name, gender, .. } = &db.person;
    let movie = db.movie.all();
    movie.with(info.with(gf_25c(db))
...
fn q26b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(complete_cast.select(subject.select(compcasttype_text).eq("cast")
                               .and(status.select(compcasttype_text).rx(r"complete")))
          .and(keyword.select(keyword_text).is_in(["superhero", "marvel-comics", "based-on-comic", "fight"]))
          .and(kind.select(kind_text).eq("movie"))
...
fn q26c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Kind { text: kind_text, .. } = &db.kind;
    let movie = db.movie.all();
    let rd = data.with(data_ty.select(infotype_text).eq("rating")).select(data_text);
    movie.with(complete_cast.select(subject.select(compcasttype_text).eq("cast")
                               .and(status.select(compcasttype_text).rx(r"complete")))
          .and(keyword.select(keyword_text).is_in(kw10()))
          .and(kind.select(kind_text).eq("movie"))

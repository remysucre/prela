...
fn q2(db: &'static Job, cc: &'static str) -> impl Drive<R: Row> {
    ...
    movie.with(keyword.select(keyword_text).eq("character-name-in-title")
          .and(company.select(country).eq(cc)))
        .select(title)
}

fn q2a(db: &'static Job) -> impl Drive<R: Row> { q2(db, "[de]") }
fn q2b(db: &'static Job) -> impl Drive<R: Row> { q2(db, "[nl]") }
fn q2c(db: &'static Job) -> impl Drive<R: Row> { q2(db, "[sm]") }
fn q2d(db: &'static Job) -> impl Drive<R: Row> { q2(db, "[us]") }
...
fn q3b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Keyword { text: keyword_text, .. } = &db.keyword;
    let movie = db.movie.all();
    movie.with(keyword.select(keyword_text).rx(r"sequel")
...
fn q4(db: &'static Job, year: i64, rating: &'static str) -> impl Drive<R: Row> {
    ...
    let movie = db.movie.all();
    movie.with(keyword.with(kw_rx(db, r"sequel"))
          .and(production_year.gt(year)))
       .select(data.with(data_ty.select(infotype_text).eq("rating")
                    .and(data_text.gt(rating))).select(data_text)
          .and(title))
}

fn q4a(db: &'static Job) -> impl Drive<R: Row> { q4(db, 2005, "5.0") }
fn q4b(db: &'static Job) -> impl Drive<R: Row> { q4(db, 2010, "9.0") }
fn q4c(db: &'static Job) -> impl Drive<R: Row> { q4(db, 1990, "2.0") }
...
fn q13a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(company.select(country.eq("[de]")
                         .and(company_ty.select(companytype_text).eq("production companies")))
          .and(kind.select(kind_text).eq("movie")))
       .select(info.with(info_ty.select(infotype_text).eq("release dates")).select(info_info)
...
fn q11a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(link
       ...
       .select(company.with(country.ne("[pl]")
                       ...
                       .and(company_ty.select(companytype_text).eq("production companies"))
                     .minus(company_note)).select(company_name)
          .and(link.select(movielink_ty).select(linktype_text).rx(r"follow"))
          .and(title))
}
...
fn q22a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(info.select(info_ty.select(infotype_text).eq("countries")
       ...
       .select(title
          .and(data.with(data_text.lt("7.0")
...
fn q1a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let InfoType { text: infotype_text, .. } = &db.info_type;
    let movie = db.movie.all();
    movie.with(data.select(data_ty).select(infotype_text).eq("top 250 rank"))
       .select(company.with(company_ty.select(companytype_text).eq("production companies")
                       .and(company_note.nrx(r"\(as Metro-Goldwyn-Mayer Pictures\)"))
                       .and(company_note.rx(r"\(co-production\)|\(presents\)"))).select(company_note)
          .and(title)
          .and(production_year))
}
...
fn q12a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let InfoType { text: infotype_text, .. } = &db.info_type;
    let movie = db.movie.all();
    movie.with(info.select(info_ty.select(infotype_text).eq("genres")
       ...
       .select(company.with(country.eq("[us]")
          ...
          .and(data.with(data_ty.select(infotype_text).eq("rating")
                    .and(data_text.gt("8.0"))).select(data_text))
          .and(title))
}
...
fn q14a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Kind { text: kind_text, .. } = &db.kind;
    let movie = db.movie.all();
    movie.with(keyword.select(keyword_text).is_in(murder4())
          .and(kind.select(kind_text).eq("movie"))
          .and(info.select(info_ty.select(infotype_text).eq("countries")
                      .and(info_info.is_in(["Sweden","Norway","Germany","Denmark","Swedish","Denish","Norwegian","German","USA","American"]))))
          .and(production_year.gt(2010)))
       .select(data.with(data_ty.select(infotype_text).eq("rating")
                    .and(data_text.lt("8.5"))).select(data_text)
          .and(title))
}
...
fn q1b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let InfoType { text: infotype_text, .. } = &db.info_type;
    let movie = db.movie.all();
    movie.with(data.select(data_ty).select(infotype_text).eq("bottom 10 rank")
          .and(production_year.ge(2005))
          .and(production_year.le(2010)))
       .select(company.with(company_ty.select(companytype_text).eq("production companies")
                       .and(company_note.nrx(r"\(as Metro-Goldwyn-Mayer Pictures\)"))).select(company_note)
          .and(title)
          .and(production_year))
}
...
fn q3a(db: &'static Job) -> impl Drive<R: Row> { q3ac(db, nordic8(), 2005) }
fn q3c(db: &'static Job) -> impl Drive<R: Row> {
    q3ac(db, vec!["Sweden","Norway","Germany","Denmark","Swedish","Denish","Norwegian","German","USA","American"], 1990)
}
...
fn q11b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(link
       ...
       .select(company.with(country.ne("[pl]")
                       ...
                       .and(company_ty.select(companytype_text).eq("production companies"))
                     .minus(company_note)).select(company_name)
          .and(link.select(movielink_ty).select(linktype_text).rx(r"follows"))
          .and(title))
}
...
fn q1c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(data.select(data_ty).select(infotype_text).eq("top 250 rank")
       ...
       .select(company.with(company_ty.select(companytype_text).eq("production companies")
                       .and(company_note.nrx(r"\(as Metro-Goldwyn-Mayer Pictures\)"))
                       .and(company_note.rx(r"\(co-production\)"))).select(company_note)
          .and(title)
          .and(production_year))
}
...
fn q12b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(company.select(country.eq("[us]")
                         .and(company_ty.select(companytype_text).is_in(["production companies", "distributors"])))
          .and(data.select(data_ty).select(infotype_text).eq("bottom 10 rank"))
          .and(production_year.gt(2000))
          .and(title.rx(r"^Birdemic|Movie")))
       .select(info.with(info_ty.select(infotype_text).eq("budget")).select(info_info)
          .and(title))
}
...
fn q12c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    movie.with(info.select(info_ty.select(infotype_text).eq("genres")
       ...
       .select(company.with(country.eq("[us]")
                       .and(company_ty.select(companytype_text).eq("production companies"))).select(company_name)
          .and(data.with(data_ty.select(infotype_text).eq("rating")
                    .and(data_text.gt("7.0"))).select(data_text))
          .and(title))
}
...
fn q14b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Kind { text: kind_text, .. } = &db.kind;
    let movie = db.movie.all();
    movie.with(keyword.select(keyword_text).is_in(["murder", "murder-in-title"])
          .and(kind.select(kind_text).eq("movie"))
          .and(info.select(info_ty.select(infotype_text).eq("countries")
                      .and(info_info.is_in(["Sweden","Norway","Germany","Denmark","Swedish","Denish","Norwegian","German","USA","American"]))))
          .and(production_year.gt(2010))
       ...
       .select(data.with(data_ty.select(infotype_text).eq("rating")
                    .and(data_text.gt("6.0"))).select(data_text)
          .and(title))
}

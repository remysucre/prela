...
pub const ENTRIES: &[super::Entry] = &[
    ...
    ("13a", "Afghanistan:24 June 2012 || 1.0 || &Me", |db| { min_row(q13a(db)) }),
    ("11a", "Churchill Films || followed by || Batman Beyond", |db| min_row(q11a(db)), ),
    ("22a", "(empty)", |db| min_row(q22a(db))),
    ("1a", "(A Warner Bros.-First National Picture) (presents) || A Clockwork Orange || 1934", |db| min_row(q1a(db)), ),
    ("5a", "(empty)", |db| min_row(q5a(db))),
    ("12a", "10th Grade Reunion Films || 8.1 || 3:20", |db| { min_row(q12a(db)) }),
    ("14a", "1.0 || $lowdown", |db| min_row(q14a(db))),
    ("1b", "(Set Decoration Rentals) (uncredited) || Disaster Movie || 2008", |db| min_row(q1b(db)), ),
    ...
    ("11b", "Filmlance International AB || follows || The Money Man", |db| min_row(q11b(db)), ),
    ("13b", "501audio || 1.8 || 5 Time Champion", |db| { min_row(q13b(db)) }),
    ("1c", "(co-production) || Intouchables || 2011", |db| { min_row(q1c(db)) }),
    ("1d", "(Set Decoration Rentals) (uncredited) || Disaster Movie || 2004", |db| min_row(q1d(db)), ),
    ("4c", "2.1 || & Teller 2", |db| min_row(q4c(db))),
    ("12b", "$10,000 || Birdemic: Shock and Terror", |db| { min_row(q12b(db)) }),
    ("12c", "\"Oh That Gus!\" || 7.1 || $1.11", |db| { min_row(q12c(db)) }),
    ...
    ("7a", "Antonioni, Michelangelo || Dressed to Kill", |db| { min_row(q7a(db)) }),
    ("7b", "De Palma, Brian || Dressed to Kill", |db| { min_row(q7b(db)) }),
    ("7c",
    ...
    ("8a", "Chambers, Linda || .hack//Quantum", |db| { min_row(q8a(db)) }),
    ("8b", "Chambers, Linda || Dragon Ball Z: Shin Budokai", |db| min_row(q8b(db)), ),
    ("8c", "\"A.J.\" || #1 Cheerleader Camp", |db| { min_row(q8c(db)) }),
    ("8d", "\"Jenny from the Block\" || #1 Cheerleader Camp", |db| min_row(q8d(db)), ),
    ("9a", "AJ || Airport Announcer || Blue Harvest", |db| { min_row(q9a(db)) }),
    ("9b", "AJ || Airport Announcer || Bassett, Angela || Blue Harvest", |db| min_row(q9b(db)), ),
    ("9c", "'Annette' || 2nd Balladeer || Alborg, Ana Esther || (1975-01-20)", |db| min_row(q9c(db)), ),
    ("9d", "!!!, Toy || Aaron, Caroline || \"Cockamamie's\" Salesgirl || $15,000.00 Error", |db| min_row(q9d(db)), ),
    ("10a", "Actor || 12 Rounds", |db| min_row(q10a(db))),
    ("10b", "(empty)", |db| min_row(q10b(db))),
    ("10c", "Himself || Evil Eyes: Behind the Scenes", |db| { min_row(q10c(db)) }),
    // --- templates 16-18 ---
    ("16a", "Adams, Stan || Carol Burnett vs. Anthony Perkins", |db| min_row(q16a(db)), ),
    ("16b", "!!!, Toy || & Teller", |db| min_row(q16b(db))),
    ("16c", "\"Brooklyn\" Tony Danza || (#1.5)", |db| { min_row(q16c(db)) }),
    ("16d", "\"Brooklyn\" Tony Danza || (#1.5)", |db| { min_row(q16d(db)) }),
    ...
    ("17f", "'El Galgo PornoStar', Blanquito", |db| { min_row(q17f(db)) }),
    ("18a", "$1,000 || 10 || 40 Days and 40 Nights", |db| { min_row(q18a(db)) }),
    ("18b", "Horror || 8.1 || Agorable", |db| min_row(q18b(db))),
    ("18c", "Action || 10 || #PostModem", |db| min_row(q18c(db))),
    // --- 19a-26c ---
    ("19a", "Angeline, Moriah || Blue Harvest", |db| { min_row(q19a(db)) }),
    ("19b", "Jolie, Angelina || Kung Fu Panda", |db| { min_row(q19b(db)) }),
    ("19c", "Alborg, Ana Esther || .hack//Akusei heni vol. 2", |db| min_row(q19c(db)), ),
    ("19d", "Aaron, Caroline || $9.99", |db| min_row(q19d(db))),
    ("20a", "Disaster Movie", |db| min_row(q20a(db))),
    ("20b", "Iron Man", |db| min_row(q20b(db))),
    ("20c", "Abell, Alistair || ...And Then I...", |db| { min_row(q20c(db)) }),
    ("21a", "Det Danske Filminstitut || followed by || Der Serienkiller - Klinge des Todes", |db| min_row(q21a(db)), ),
    ("21b", "Filmlance International AB || followed by || Hämndens pris", |db| min_row(q21b(db)), ),
    ("21c", "Churchill Films || followed by || Batman Beyond", |db| min_row(q21c(db)), ),
    ...
    ("24a", "Additional Voices || Baker, Andrea || Baiohazâdo 6", |db| min_row(q24a(db)), ),
    ("24b", "Tigress || Jolie, Angelina || Kung Fu Panda 2", |db| min_row(q24b(db)), ),
    ("25a", "Horror || 10 || -- And Now the Screaming Starts! || Abdallah, Damon", |db| min_row(q25a(db)), ),
    ("25b", "Horror || 138 || Vampire Boys || Campbell, Jeremiah", |db| min_row(q25b(db)), ),
    ("25c", "Action || 10 || $ || Aakeson, Kim Fupz", |db| { min_row(q25c(db)) }),
    ("26a", "'Agua' Man || Acereda, Hermie || 7.1 || 3:10 to Yuma", |db| min_row(q26a(db)), ),
    ("26b", "Bank Manager || 8.2 || Inception", |db| { min_row(q26b(db)) }),
    ("26c", "'Agua' Man || 1.9 || 12 Rounds", |db| { min_row(q26c(db)) }),
    // --- 27a-33c ---
...
fn q2(db: &'static Job, cc: &'static str) -> impl Drive<R: Row> {
    let Movie { title, keyword, company, .. } = &db.movie;
...
fn q3b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, info, .. } = &db.movie;
...
fn q4(db: &'static Job, year: i64, rating: &'static str) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, data, .. } = &db.movie;
    let Data { text: data_text, ty: data_ty, .. } = &db.data;
    db.movie.with(keyword.rx_dict(r"sequel")
                  .and(production_year.gt(year)))
        .select(data.with(data_ty.eq("rating"))
                    .select(data_text.gt(rating))
                .and(title))
}
...
fn q13a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Company { country, ty: company_ty, .. } = &db.company;
    let Data { text: data_text, ty: data_ty, .. } = &db.data;
...
fn q11a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Company { name, country, note, ty, .. } = &db.company;
    let MovieLink { ty: link_ty, .. } = &db.movie_link;
    db.movie.with(keyword.eq("sequel")
                  .and(production_year.between(1950, 2000)))
            .select(company.with(country.ne("[pl]")
                                 .and(name.rx(r"Film|Warner"))
                                 .and(ty.eq("production companies"))
                                 .minus(note))
                           .select(name)
...
fn q22a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Data { text: data_text, ty: data_ty, .. } = &db.data;
    let Info { info: info_info, ty: info_ty, .. } = &db.info;
    db.movie.with(info.select(info_ty.eq("countries")
            ...
            .select(title
                    .and(data.with(data_ty.eq("rating"))
                             .select(data_text.lt("7.0")))
                    .and(company.with(company_note.nrx(r"\(USA\)")
                                                  .rx(r"\(200.*\)")
                                      .and(country.ne("[us]"))
                                      .and(company_ty.eq("production companies")))
                                .select(company_name)))
...
fn q1a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, data, .. } = &db.movie;
    let Company { note, ty: ct, .. } = &db.company;
    let Data { ty: dt, .. } = &db.data;
    db.movie.with(data.select(dt).eq("top 250 rank"))
            .select(company.with(ct.eq("production companies"))
                           .select(note.nrx(r"\(as Metro-Goldwyn-Mayer Pictures\)")
...
fn q5a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Company { note, ty, .. } = &db.company;
    let Info { info: info_info, .. } = &db.info;
    db.movie.with(company.select(ty.eq("production companies")
                                 .and(note.rx(r"\(theatrical\)").rx(r"\(France\)")))
...
fn q12a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, info, data, .. } = &db.movie;
    let Company { name, country, ty: company_ty, .. } = &db.company;
    let Data { text: data_text, ty: data_ty, .. } = &db.data;
    let Info { info: info_info, ty: info_ty, .. } = &db.info;
    db.movie.with(info.select(info_ty.eq("genres")
...
fn q14a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, kind, production_year, keyword, info, data, .. } = &db.movie;
    let Data { text: data_text, ty: data_ty, .. } = &db.data;
    let Info { info: info_info, ty: info_ty, .. } = &db.info;
    db.movie.with(keyword.is_in(murder4())
                  .and(kind.eq("movie"))
                  .and(info.select(info_ty.eq("countries")
...
fn q1b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, data, .. } = &db.movie;
    let Company { note, ty: company_ty, .. } = &db.company;
...
fn q3c(db: &'static Job) -> impl Drive<R: Row> {
    q3ac(db, vec!["Sweden", "Norway", "Germany", "Denmark", "Swedish",
                  "Denish", "Norwegian", "German", "USA", "American"], 1990)
}
...
fn q22b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, kind, production_year, keyword, company, info, data, .. } = &db.movie;
    let Company { name, country, note, ty: company_ty, .. } = &db.company;
    let Data { text: data_text, ty: data_ty, .. } = &db.data;
    let Info { info: info_info, ty: info_ty, .. } = &db.info;
    db.movie.with(info.select(info_ty.eq("countries")
                              .and(info_info.is_in(["Germany", "German", "USA", "American"])))
            ...
            .select(title
                    ...
                    .and(company.with(note.nrx(r"\(USA\)").rx(r"\(200.*\)")
                                      .and(country.ne("[us]"))
                                      .and(company_ty.eq("production companies")))
                                .select(name)))
}
...
fn q22c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Data { text: data_text, ty: data_ty, .. } = &db.data;
    let Info { info: info_info, ty: info_ty, .. } = &db.info;
    db.movie.with(info.select(info_ty.eq("countries").and(info_info.is_in(nordic10())))
                  .and(keyword.is_in(murder4()))
                  .and(production_year.gt(2005))
                  .and(kind.is_in(["movie", "episode"])))
            .select(title
...
fn q5b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Info { info: info_info, .. } = &db.info;
    db.movie.with(company.select(ty.eq("production companies")
                                 .and(note.rx(r"\(VHS\)").rx(r"\(USA\)").rx(r"\(1994\)")))
...
fn q15b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, company, info, aka, .. } = &db.movie;
    let Company { name, country, note: company_note, .. } = &db.company;
    let Info { info: info_info, ty, note: info_note, .. } = &db.info;
    db.movie.with(company.select(country.eq("[us]")
                                 .and(name.eq("YouTube"))
                                 .and(company_note.rx(r"\(200.*\)").rx(r"\(worldwide\)")))
...
fn q15c(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, company, info, aka, .. } = &db.movie;
    let Company { country, .. } = &db.company;
    let Info { info: info_info, ty, note, .. } = &db.info;
    db.movie.with(company.select(country).eq("[us]")
                  .and(keyword)
                  .and(aka)
                  .and(production_year.gt(1990)))
            .select(info.with(ty.eq("release dates")
                              .and(note.rx(r"internet")))
...
fn q15d(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, company, info, aka, .. } = &db.movie;
    let Company { country, .. } = &db.company;
    let Info { ty, note, .. } = &db.info;
    db.movie.with(company.select(country).eq("[us]")
                  .and(keyword)
                  .and(info.select(ty.eq("release dates").and(note.rx(r"internet"))))
                  .and(production_year.gt(1990)))
...
fn q11c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(keyword.is_in(["sequel", "revenge", "based-on-novel"])
                  ...
                  .and(link))
            .select(company.with(country.ne("[pl]")
                                 .and(name.rx(r"^20th Century Fox|^Twentieth Century Fox"))
                                 .and(ty.ne("production companies"))
                                 .and(note))
                           .select(name.and(note))
                    .and(title))
}
...
fn q6_marvel(db: &'static Job, year: i64) -> impl Drive<R: Row> {
    ...
    let downey = cast.select(person).select(name).rx(r"Downey.*Robert");
    db.movie.with(production_year.gt(year)
                  .and(kw()))
            .select(kw()
                    .and(title)
                    .and(downey))
...
fn q6f(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Person { name, .. } = &db.person;
    let kw = || keyword.is_in(kw8());
    let cast_name = cast.select(person).select(name);
    db.movie.with(production_year.gt(2000)
                  .and(kw()))
            .select(kw()
                    .and(title)
...
fn q7a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, cast, linked_by, .. } = &db.movie;
    let Cast { person, .. } = &db.cast;
    let MovieLink { ty: link_ty, .. } = &db.movie_link;
    let Person { name, gender, alias, bio, name_pcode_cf, .. } = &db.person;
    let PersonInfo { ty: personinfo_ty, note, .. } = &db.person_info;
    db.movie.with(production_year.between(1980, 1995)
            ...
            .select(cast.select(person.with(alias.rx(r"a")
                                            ...
                                            .and(gender.is_in(["m", "f"])
                                                       .minus(gender.eq("f").minus(name.rx(r"^B"))))
                                            .and(bio.select(personinfo_ty.eq("mini biography")
                                                            .and(note.eq("Volker Boehm")))))
                                      .select(name))
                    .and(title))
}
...
fn bio_filter_7c(db: &'static Job) -> impl Query<D = Id<PersonInfo>> + Probe {
    let PersonInfo { ty, note, .. } = &db.person_info;
    ty.eq("mini biography").and(note)
}

fn q7c(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { production_year, cast, linked_by, .. } = &db.movie;
    let Cast { person, .. } = &db.cast;
    let MovieLink { ty, .. } = &db.movie_link;
    let Person { name, gender, alias, bio, name_pcode_cf, .. } = &db.person;
    let PersonInfo { info, .. } = &db.person_info;
    db.movie.with(production_year.between(1980, 2010)
                  .and(linked_by.select(ty).is_in(["references", "referenced in", "features", "featured in"])))
            .select(cast.select(person.with(alias.rx(r"a|^A")
                                            .and(name_pcode_cf.between("A", "F"))
                                            // m ∨ (f ∧ name~^A), spelled {m,f} ∖ (f ∖ ^A):
                                            // ∨ is member-only and can't sit inside a probed ∧-tree.
                                            .and(gender.is_in(["m", "f"])
                                                       .minus(gender.eq("f").minus(name.rx(r"^A"))))
                                            .and(bio.with(bio_filter_7c(db))))
                                      .select(name
                                              .and(bio.with(bio_filter_7c(db)).select(info)))))
}

fn q8a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, company, cast, .. } = &db.movie;
    let Cast { person, role, note: cast_note, .. } = &db.cast;
    let Company { country, note: company_note, .. } = &db.company;
    let Person { name, alias, .. } = &db.person;
    db.movie.with(company.select(country.eq("[jp]")
                                 .and(company_note.rx(r"\(Japan\)").nrx(r"\(USA\)"))))
            .select(cast.with(cast_note.eq("(voice: English version)")
                              .and(role.eq("actress"))
                              .and(person.with(name.rx(r"Yo").nrx(r"Yu"))))
                        .select(person).select(alias)
                    .and(title))
}

fn q8b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, cast, .. } = &db.movie;
    let Cast { person, role, note: cast_note, .. } = &db.cast;
    let Company { country, note: company_note, .. } = &db.company;
    let Person { name, alias, .. } = &db.person;
    db.movie.with(company.select(country.eq("[jp]")
...
fn q8cd(db: &'static Job, role_: &'static str) -> impl Drive<R: Row> {
    ...
    db.movie.with(company.select(country).eq("[us]"))
            .select(cast.with(role.eq(role_)).select(person).select(alias)
                    .and(title))
}
...
fn q9b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(company.select(country.eq("[us]")
            ...
            .select(cast.with(cast_note.eq("(voice)")
                              .and(role.eq("actress"))
                              .and(person.with(gender.eq("f").and(name.rx(r"Angel")))))
                        .select(person.select(alias)
                                .and(character)
                                .and(person.select(name)))
                    .and(title))
}

fn q9c(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, company, cast, .. } = &db.movie;
    let Cast { person, role, note, character, .. } = &db.cast;
    let Company { country, .. } = &db.company;
    let Person { name, gender, alias, .. } = &db.person;
    db.movie.with(company.select(country).eq("[us]"))
            .select(cast.with(note.is_in(voice4())
                              .and(role.eq("actress"))
                              .and(person.with(gender.eq("f").and(name.rx(r"An")))))
                        .select(person.select(alias)
                                .and(character)
                                .and(person.select(name)))
                    .and(title))
...
fn q10a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, cast, .. } = &db.movie;
    let Cast { role, note, character, .. } = &db.cast;
    let Company { country, .. } = &db.company;
    db.movie.with(company.select(country).eq("[ru]")
                  .and(production_year.gt(2005)))
            .select(cast.with(note.rx(r"\(voice\)").rx(r"\(uncredited\)")
                              .and(role.eq("actor")))
                        .select(character)
                    .and(title))
}
...
fn q10b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(company.select(country).eq("[ru]")
                  .and(production_year.gt(2010)))
            .select(cast.with(note.rx(r"\(producer\)")
                              .and(role.eq("actor")))
                        .select(character)
                    .and(title))
}
...
fn q10c(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, cast, .. } = &db.movie;
    let Cast { note, character, .. } = &db.cast;
    let Company { country, .. } = &db.company;
    db.movie.with(company.select(country).eq("[us]")
                  .and(production_year.gt(1990)))
            .select(cast.with(note.rx(r"\(producer\)")).select(character)
                    .and(title))
}
...
fn q16ad(db: &'static Job, lo: i64) -> impl Drive<R: Row> {
    let Movie { title, episode_nr, keyword, company, cast, .. } = &db.movie;
    let Cast { person, .. } = &db.cast;
    let Company { country, .. } = &db.company;
    let Person { alias, .. } = &db.person;
    db.movie.with(company.select(country).eq("[us]")
                  .and(keyword.eq("character-name-in-title"))
                  .and(episode_nr.ge(lo).lt(100)))
...
fn q16b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, keyword, company, cast, .. } = &db.movie;
    let Cast { person, .. } = &db.cast;
    let Company { country, .. } = &db.company;
    let Person { alias, .. } = &db.person;
    db.movie.with(company.select(country).eq("[us]")
...
fn q17a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { keyword, company, cast, .. } = &db.movie;
    let Cast { person, .. } = &db.cast;
    let Company { country, .. } = &db.company;
    let Person { name, .. } = &db.person;
    db.movie.with(company.select(country).eq("[us]")
                  .and(keyword.eq("character-name-in-title")))
            .select(cast.select(person).select(name).rx(r"^B"))
}
...
fn q17_any_co(db: &'static Job, re: &str) -> impl Drive<R: Row> {
    ...
    db.movie.with(company
                  .and(keyword.eq("character-name-in-title")))
            .select(cast.select(person).select(name).rx(re))
}
...
fn q17e(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(company.select(country).eq("[us]")
                  .and(keyword.eq("character-name-in-title")))
            .select(cast.select(person).select(name))
}

fn ib_18a(db: &'static Job) -> impl Query<R = &'static str, D = Id<Movie>> + Drive + Probe {
    let Movie { info, .. } = &db.movie;
    let Info { info: info_info, ty, .. } = &db.info;
    info.with(ty.eq("budget")).select(info_info)
}

fn q18a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, cast, data, .. } = &db.movie;
    let Cast { person, note, .. } = &db.cast;
    let Data { text: data_text, ty, .. } = &db.data;
    let Person { name, gender, .. } = &db.person;
    db.movie.with(ib_18a(db)
                  .and(cast.select(note.is_in(["(producer)", "(executive producer)"])
                                   .and(person.select(gender.eq("m").and(name.rx(r"Tim")))))))
            .select(ib_18a(db)
                    .and(data.with(ty.eq("votes")).select(data_text))
                    .and(title))
}
...
fn gf_18b(db: &'static Job) -> impl Query<D = Id<Info>> + Probe {
    let Info { info, ty, note, .. } = &db.info;
    ty.eq("genres").and(info.is_in(["Horror", "Thriller"])).minus(note)
}

fn q18b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, cast, info, data, .. } = &db.movie;
    let Cast { person, note, .. } = &db.cast;
    let Data { text: data_text, ty, .. } = &db.data;
    let Info { info: info_info, .. } = &db.info;
    let Person { gender, .. } = &db.person;
    db.movie.with(info.with(gf_18b(db))
                  .and(production_year.between(2008, 2014))
                  .and(cast.select(note.is_in(writer5())
                                   .and(person.select(gender.eq("f"))))))
            .select(info.with(gf_18b(db)).select(info_info)
                    .and(data.with(ty.eq("rating")).select(data_text.gt("8.0")))
                    .and(title))
}

fn gf_18c(db: &'static Job) -> impl Query<D = Id<Info>> + Probe {
    let Info { info, ty, .. } = &db.info;
    ty.eq("genres").and(info.is_in(genre6()))
}

fn q18c(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, cast, info, data, .. } = &db.movie;
    let Cast { person, note, .. } = &db.cast;
    let Data { text: data_text, ty, .. } = &db.data;
    let Info { info: info_info, .. } = &db.info;
    let Person { gender, .. } = &db.person;
    db.movie.with(info.with(gf_18c(db))
                  .and(cast.select(note.is_in(writer5())
                                   .and(person.select(gender.eq("m"))))))
            .select(info.with(gf_18c(db)).select(info_info)
                    .and(data.with(ty.eq("votes")).select(data_text))
                    .and(title))
}
...
fn gf_25ab(db: &'static Job) -> impl Query<D = Id<Info>> + Probe {
    let Info { info, ty, .. } = &db.info;
    ty.eq("genres").and(info.eq("Horror"))
}
...
fn q19a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, company, cast, info, .. } = &db.movie;
    let Cast { person, role, note: cast_note, character, .. } = &db.cast;
    let Company { country, note: company_note, .. } = &db.company;
    let Info { info: info_info, ty, .. } = &db.info;
    let Person { name, gender, alias, .. } = &db.person;
    db.movie.with(company.select(country.eq("[us]")
                                 .and(company_note.rx(r"\(USA\)|\(worldwide\)")))
                  .and(info.select(ty.eq("release dates")
...
fn q20a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, kind, production_year, keyword, cast, complete_cast, .. } = &db.movie;
    let Cast { character, .. } = &db.cast;
    let CompleteCast { status, subject, .. } = &db.complete_cast;
    db.movie.with(complete_cast.select(subject.eq("cast").and(status.rx(r"complete")))
...
fn q20b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, kind, production_year, keyword, cast, complete_cast, .. } = &db.movie;
    let Cast { person, character, .. } = &db.cast;
    let CompleteCast { status, subject, .. } = &db.complete_cast;
    let Person { name, .. } = &db.person;
    db.movie.with(complete_cast.select(subject.eq("cast").and(status.rx(r"complete")))
                  .and(keyword.is_in(kw8()))
                  ...
                  .and(production_year.gt(2000))
                  .and(cast.select(character.nrx(r"Sherlock").rx(r"Tony.*Stark|Iron.*Man")
                                   .and(person.select(name).rx(r"Downey.*Robert")))))
            .select(title)
}
...
fn q20c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(complete_cast.select(subject.eq("cast").and(status.rx(r"complete")))
                  ...
                  .and(kind.eq("movie"))
                  .and(production_year.gt(2000)))
            .select(cast.with(character.rx(r"[Mm]an")).select(person).select(name)
                    .and(title))
}
...
fn q21(db: &'static Job, countries: Vec<&'static str>, ylo: i64, yhi: i64) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, info, link, .. } = &db.movie;
    let Company { name, .. } = &db.company;
    let Info { info: info_info, .. } = &db.info;
    db.movie.with(link.and(keyword.eq("sequel"))
                  .and(production_year.between(ylo, yhi))
                  .and(info.select(info_info).is_in(countries))
                  .and(follow_link(db)))
            .select(film_or_warner_co(db).select(name)
                    .and(follow_link(db))
                    .and(title))
}
...
fn q23a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, company, info, complete_cast, .. } = &db.movie;
    let Company { country, .. } = &db.company;
...
fn q24a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, company, cast, info, .. } = &db.movie;
    let Cast { person, role, note, character, .. } = &db.cast;
    let Company { country, .. } = &db.company;
    let Info { info: info_info, ty, .. } = &db.info;
    let Person { name, gender, alias, .. } = &db.person;
    db.movie.with(company.select(country).eq("[us]")
            ...
            .select(cast.with(note.is_in(voice4())
                              ...
                              .and(person.with(gender.eq("f")
                                               .and(name.rx(r"An"))
                                               .and(alias))))
                        .select(character.and(person.select(name)))
                    .and(title))
}
...
fn q24b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Info { info: info_info, ty, .. } = &db.info;
    let Person { name: person_name, gender, alias, .. } = &db.person;
    db.movie.with(company.select(country.eq("[us]")
...
fn q25a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, keyword, cast, info, data, .. } = &db.movie;
    let Cast { person, note, .. } = &db.cast;
    let Data { text: data_text, ty, .. } = &db.data;
    let Info { info: info_info, .. } = &db.info;
    let Person { name, gender, .. } = &db.person;
    db.movie.with(info.with(gf_25ab(db))
                  .and(keyword.is_in(["murder", "blood", "gore", "death", "female-nudity"])))
            .select(info.with(gf_25ab(db)).select(info_info)
                    .and(data.with(ty.eq("votes")).select(data_text))
                    .and(title)
                    .and(cast.with(note.is_in(writer5())
                                   .and(person.with(gender.eq("m"))))
                             .select(person).select(name)))
}

fn q25b(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, production_year, keyword, cast, info, data, .. } = &db.movie;
    let Cast { person, note, .. } = &db.cast;
    let Data { text: data_text, ty, .. } = &db.data;
    let Info { info: info_info, .. } = &db.info;
    let Person { name, gender, .. } = &db.person;
...
fn q25c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Info { info: info_info, .. } = &db.info;
    let Person { name, gender, .. } = &db.person;
    db.movie.with(info.with(gf_25c(db))
                  .and(keyword.is_in(kw7())))
            .select(info.with(gf_25c(db)).select(info_info)
                    .and(data.with(ty.eq("votes")).select(data_text))
                    .and(title)
                    .and(cast.with(note.is_in(writer5())
                                   .and(person.with(gender.eq("m"))))
...
fn q26a(db: &'static Job) -> impl Drive<R: Row> {
    let Movie { title, kind, production_year, keyword, cast, data, complete_cast, .. } = &db.movie;
    let Cast { person, character, .. } = &db.cast;
    let CompleteCast { status, subject, .. } = &db.complete_cast;
    let Data { text: data_text, ty, .. } = &db.data;
    let Person { name, .. } = &db.person;
    db.movie.with(complete_cast.select(subject.eq("cast").and(status.rx(r"complete")))
                  ...
                  .and(kind.eq("movie"))
                  .and(production_year.gt(2000)))
            .select(cast.with(character.rx(r"[Mm]an")).select(character.and(person.select(name)))
                    .and(data.with(ty.eq("rating")).select(data_text.gt("7.0")))
                    .and(title))
}
...
fn q26c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let CompleteCast { status, subject, .. } = &db.complete_cast;
    let Data { text: data_text, ty, .. } = &db.data;
    let rd = data.with(ty.eq("rating")).select(data_text);
    db.movie.with(complete_cast.select(subject.eq("cast").and(status.rx(r"complete")))
                  .and(keyword.is_in(kw10()))
                  .and(kind.eq("movie"))
                  .and(production_year.gt(2000)))
...
fn gf_horror(db: &'static Job) -> impl Query<D = Id<Info>> + Probe {
    let Info { info, ty, .. } = &db.info;
    ty.eq("genres").and(info.is_in(["Horror", "Thriller"]))
}
...
fn q27a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(link.and(keyword.eq("sequel"))
                  ...
                  .and(complete_cast.select(subject.is_in(["cast", "crew"]).and(status.eq("complete"))))
                  .and(follow_link(db)))
            .select(film_or_warner_co(db).select(name)
                    .and(follow_link(db))
                    .and(title))
...
fn q28a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Info { info: info_info, ty, .. } = &db.info;
    db.movie.with(complete_cast.select(subject.eq("crew").and(status.ne("complete+verified")))
                  .and(co_28(db))
                  .and(info.select(ty.eq("countries").and(info_info.is_in(nordic10()))))
                  .and(dt_28ac(db))
                  .and(keyword.is_in(murder4()))
                  .and(kind.is_in(["movie", "episode"]))
                  .and(production_year.gt(2000)))
            .select(co_28(db).select(name)
...
fn q28b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(complete_cast.select(subject.eq("crew").and(status.ne("complete+verified")))
                  .and(co_28(db))
                  .and(info.select(ty.eq("countries")
...
fn q28c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Data { text: data_text, .. } = &db.data;
    let Info { info: info_info, ty, .. } = &db.info;
    db.movie.with(complete_cast.select(subject.eq("cast").and(status.eq("complete")))
                  .and(co_28(db))
                  .and(info.select(ty.eq("countries").and(info_info.is_in(nordic10()))))
                  .and(dt_28ac(db))
                  .and(keyword.is_in(murder4()))
...
fn q29a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let CompleteCast { status, subject, .. } = &db.complete_cast;
    let Info { info: info_info, ty: info_ty, .. } = &db.info;
    let Person { name, gender, alias, bio, .. } = &db.person;
    let PersonInfo { ty: personinfo_ty, .. } = &db.person_info;
    db.movie.with(complete_cast.select(subject.eq("cast").and(status.eq("complete+verified")))
                  .and(company.select(country).eq("[us]"))
                  .and(info.select(info_ty.eq("release dates")
                                   .and(info_info.rx(r"^Japan:.*200|^USA:.*200"))))
                  ...
                  .and(production_year.between(2000, 2010)))
            .select(cast.with(note.is_in(voice3())
                              .and(role.eq("actress"))
...
fn q30a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Info { info: info_info, .. } = &db.info;
    let Person { name, gender, .. } = &db.person;
    db.movie.with(complete_cast.select(subject.is_in(["cast", "crew"]).and(status.eq("complete+verified")))
...
fn q31c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(company.select(company_name).rx(r"^Lionsgate")
            ...
            .select(info.with(gf_genre6(db)).select(info_info)
                    .and(data.with(ty.eq("votes")).select(data_text))
                    .and(title)
                    .and(cast.with(note.is_in(writer5())).select(person).select(person_name)))
}
...
fn q32(db: &'static Job, kw: &'static str) -> impl Drive<R: Row> {
    ...
    let MovieLink { target, ty, .. } = &db.movie_link;
    db.movie.with(link.and(keyword.eq(kw)))
            .select(link.select(ty)
                    .and(title)
...
fn q33a(db: &'static Job) -> impl Drive<R: Row> {
    ...
    let Company { name, country, .. } = &db.company;
    let Data { text: data_text, ty, .. } = &db.data;
    let MovieLink { target, .. } = &db.movie_link;
    db.movie.with(kind.eq("tv series")
                  .and(company.select(country).eq("[us]"))
                  .and(qlink_33a(db)))
            .select(company.with(country.eq("[us]")).select(name)
                    .and(qlink_33a(db).select(target).select(company).select(name))
                    .and(data.with(ty.eq("rating")).select(data_text))
                    .and(qlink_33a(db).select(target)
                                      .select(data.with(ty.eq("rating")).select(data_text.lt("3.0"))))
                    .and(title)
...
fn q33b(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(kind.eq("tv series")
                  ...
                  .and(qlink_33b(db)))
            .select(company.with(country.eq("[nl]")).select(name)
                    .and(qlink_33b(db).select(target).select(company).select(name))
                    .and(data.with(ty.eq("rating")).select(data_text))
                    .and(qlink_33b(db).select(target)
                                      .select(data.with(ty.eq("rating")).select(data_text.lt("3.0"))))
                    .and(title)
...
fn q33c(db: &'static Job) -> impl Drive<R: Row> {
    ...
    db.movie.with(kind.is_in(["tv series", "episode"])
                  ...
                  .and(qlink_33c(db)))
            .select(company.with(country.ne("[us]")).select(name)
                    .and(qlink_33c(db).select(target).select(company).select(name))
                    .and(data.with(ty.eq("rating")).select(data_text))
                    .and(qlink_33c(db).select(target)
                                      .select(data.with(ty.eq("rating")).select(data_text.lt("3.5"))))
                    .and(title)

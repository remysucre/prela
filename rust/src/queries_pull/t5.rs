// PULL-PROTOCOL PORT of src/queries/t5.rs — identical plans, pull sinks.
...
fn k_23ab<'d>(d: &'d Data) -> impl Query<R = &'static str, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
    (&d.movie_kind).o(&d.kind_kind).eq("movie")
}

fn k_23c<'d>(d: &'d Data) -> impl Query<R = &'static str, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
...
fn gf_25ab<'d>(d: &'d Data) -> impl Query<D = usize> + Probe + ProbeIter + 'd {
    (&d.info_type).o(&d.infotype_info).eq("genres")
        .and((&d.info_info).eq("Horror"))
}

fn gf_25c<'d>(d: &'d Data) -> impl Query<D = usize> + Probe + ProbeIter + 'd {
    (&d.info_type).o(&d.infotype_info).eq("genres")

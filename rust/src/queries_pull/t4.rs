// PULL-PROTOCOL PORT of src/queries/t4.rs — identical plans, pull sinks.
...
fn ib_18a<'d>(d: &'d Data) -> impl Query<R = &'static str, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
...
fn gf_18b<'d>(d: &'d Data) -> impl Query<D = usize> + Probe + ProbeIter + 'd {
...
fn gf_18c<'d>(d: &'d Data) -> impl Query<D = usize> + Probe + ProbeIter + 'd {
    (&d.info_type).o(&d.infotype_info).eq("genres")

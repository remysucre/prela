// PULL-PROTOCOL PORT of src/queries/t6.rs — identical plans, pull sinks.
...
fn co_28<'d>(d: &'d Data) -> impl Query<R = usize, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
...
fn dt_28ac<'d>(d: &'d Data) -> impl Query<R = usize, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
    (&d.movie_data).in_s(
...
fn dt_28b<'d>(d: &'d Data) -> impl Query<R = usize, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
    (&d.movie_data).in_s(
        (&d.data_type).o(&d.infotype_info).eq("rating")
...
fn gf_horror<'d>(d: &'d Data) -> impl Query<D = usize> + Probe + ProbeIter + 'd {
    (&d.info_type).o(&d.infotype_info).eq("genres")
        .and((&d.info_info).in_v(vec!["Horror", "Thriller"]))
}

fn gf_genre6<'d>(d: &'d Data) -> impl Query<D = usize> + Probe + ProbeIter + 'd {
    (&d.info_type).o(&d.infotype_info).eq("genres")
        .and((&d.info_info).in_v(genre6()))
}

fn qlink_33a<'d>(d: &'d Data) -> impl Query<R = usize, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
    (&d.movie_link).in_s(
...
fn qlink_33b<'d>(d: &'d Data) -> impl Query<R = usize, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
    (&d.movie_link).in_s(
...
fn qlink_33c<'d>(d: &'d Data) -> impl Query<R = usize, D = usize> + Drive + Probe + Iterate + ProbeIter + 'd {
    (&d.movie_link).in_s(
        (&d.movielink_type).o(&d.linktype_link).in_v(link3())
            .and((&d.movielink_target).in_s(

...
impl<D: Dense> Bitset<D> {
    ...
    /// `member`: `Probe for Bitset` calls `self.member(x)`, and method
    /// resolution must find this — the blanket `Member` default is
    /// `probe_any(x, |_| true)`, which would recurse forever.
    #[inline(always)]

...
fn q1() -> String {
    ...
    let (li_shipdate, li_returnflag, li_status) = (shipdate(), returnflag(), Lineitem::status());
    let (li_quantity, li_extendedprice, li_discount, li_tax) =
        (quantity(), extendedprice(), discount(), tax());
    for li in 0..lineitem().n {
...
fn q4() -> String {
    ...
    let (li_commitdate, li_receiptdate, li_order) = (commitdate(), receiptdate(), order());
    for li in 0..lineitem().n {
...
fn q8() -> String {
    ...
    for o in 0..n_ord {
        let cust = Order::customer().values[o];
        if cust == Dense::NONE { continue; }   // sparse orderkey gap (hole fill NO_ID)
        let nation = Customer::nation().values[cust.idx()];
        let region = Nation::region().values[nation.idx()];
...
fn q13() -> String {
    ...
    let (ord_customer, ord_comment) = (Order::customer(), Order::comment());
    for o in 0..orders().n {
...
fn q18() -> String {
    ...
    let (li_order, li_quantity) = (order(), quantity());
    for li in 0..lineitem().n {
...
fn q20() -> String {
    ...
    for i in 0..n_part {
        pa_is_forest[i] = Part::name().values[i].starts_with("forest");
    }
    let n_ps = partsupp().n;
    ...
    live_li.drive(|li, _| {
        let li = li.idx();
        let part = li_part.values[li];
        if !pa_is_forest[part.idx()] { return; }
...
fn q21() -> String {
    ...
    let (li_order, li_supplier) = (order(), Lineitem::supplier());
    let (li_commitdate, li_receiptdate) = (commitdate(), receiptdate());
    for li in 0..lineitem().n {
    ...
    for s in 0..n_supp {
        let nation_id = Supplier::nation().values[s];
        is_saudi[s] = Nation::name().values[nation_id.idx()] == "SAUDI ARABIA";
    }
    let mut is_f_ord: Vec<bool> = vec![false; n_ord];
    for o in 0..n_ord {
        is_f_ord[o] = Order::status().values[o] == "F";
    }
...
fn q2() -> String {
    ...
    let (ps_part, ps_supplycost) = (PartSupp::part(), supplycost());
    (&eu_ps).drive(|ps, _| {

...
#[inline]
pub fn fmt_yyyymmdd(d: i64) -> String {
...
pub fn oracle(name: &'static str) -> &'static str {
    ...
    let map = CACHE.get_or_init(|| {
        ...
        for n in [
            "Q2", "Q7", "Q8", "Q9", "Q11", "Q13", "Q15", "Q16", "Q17", "Q18", "Q19", "Q20", "Q21",
            "Q22",
        ] {
...
fn q8() -> String {
    ...
    let result = lineitem
        ...
        .select(
            order
                .with(Order::customer.nation().region().eq("AMERICA"))
                .select(date.between(19950101, 19961231))
                .map(|d: i64| d / 10000),
        )
        .gather(
            extendedprice
...
fn q9() -> String {
    let sc: HashIdx<_, _> = supplycost
        .group_by(
            PartSupp::part
                .with(Part::name.filt(|n: &str| n.contains("green")))
...
fn q10() -> String {
    ...
    let mut rows: Vec<(
        Id<Customer>,
        (f64, (((((&str, f64), &str), &str), &str), &str)),
    )> = Vec::new();
    revenue.drive(|k, v| rows.push((k, v)));
    rows.sort_by(|(_, (r1, _)), (_, (r2, _))| r2.partial_cmp(r1).unwrap());
    rows.truncate(20);
    join_lines(rows.iter().map(
        |(c, (r, (((((name, acctbal), nat), addr), phone), comment)))| {
...
fn q19() -> String {
    ...
    let pred = Lineitem::part
        .select(brand.and(container).and(size))
        .and(quantity)
        .filt(|(((br, ct), sz), q)| {
            let in_v = |arr: &[&str], s: &str| arr.iter().any(|&a| a == s);
            (br == "Brand#12"
                && in_v(&["SM CASE", "SM BOX", "SM PACK", "SM PKG"], ct)
                && q >= 1.0
                && q <= 11.0
                && sz >= 1
                && sz <= 5)
                || (br == "Brand#23"
                    && in_v(&["MED BAG", "MED BOX", "MED PKG", "MED PACK"], ct)
                    && q >= 10.0
                    && q <= 20.0
                    && sz >= 1
                    && sz <= 10)
                || (br == "Brand#34"
                    && in_v(&["LG CASE", "LG BOX", "LG PACK", "LG PKG"], ct)
                    && q >= 20.0
                    && q <= 30.0
                    && sz >= 1
                    && sz <= 15)
    ...
    let qual_supps = Bitset::over(
        ...
        &partsupp
            .with(
                PartSupp::part
                    .name()
                    .filt(|n: &str| n.starts_with("forest"))
                    .and(
                        availqty
                            .map(|q| q as f64)
                            .and(threshold)
